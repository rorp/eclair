...
class PaymentLifecycleSpec extends BaseRouterSpec {
  ...
  test("send to route (routing hints)") { routerFixture =>
    ...
    val extraEdges = Seq(ExtraEdge(c, recipientNodeId, ShortChannelId(561), 1 msat, 100, CltvExpiryDelta(144), 1 msat, None, None))
  ...
  test("payment failed (Update in assisted route)") { routerFixture =>
    ...
    val recipient = ClearRecipient(d, Features.empty, defaultAmountMsat, defaultExpiry, defaultInvoice.paymentSecret, Seq(
      ExtraEdge(b, c, scid_bc, update_bc.feeBaseMsat, update_bc.feeProportionalMillionths, update_bc.cltvExpiryDelta, 1 msat, None, update_bc.inboundFees_opt),
      ExtraEdge(c, d, scid_cd, update_cd.feeBaseMsat, update_cd.feeProportionalMillionths, update_cd.cltvExpiryDelta, 1 msat, None, update_cd.inboundFees_opt)
    ))
