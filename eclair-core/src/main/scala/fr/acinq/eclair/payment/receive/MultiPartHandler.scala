          ...
          case CreateBolt12Invoice(r) =>
            ...
            context.pipeToSelf(Future.sequence(r.routes.map(route => {
              val dummyHops = route.dummyHops.map(h => {
                ...
                val edge = Invoice.ExtraEdge(nodeParams.nodeId, nodeParams.nodeId, ShortChannelId.toSelf, h.feeBase, h.feeProportionalMillionths, h.cltvExpiryDelta, htlcMinimum = 1 msat, htlcMaximum_opt = None, None)
